...
const smoothSaturation = (saturation: number, luminance: number): number => {
...
const smoothLightness = (lightness: number, luminance: number): number => {
    const isBright = luminance > 0.6;
...
export function getLightVariant(color: string): string {
